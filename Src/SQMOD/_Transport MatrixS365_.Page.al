page 50106 "Transport MatrixS365"
{
    ...
    begin
        ...
        for I:=1 to ArrayLen(MatrixColumnDateFilters)do begin
            ...
            if MatrixRec.Find('-')then repeat if JobNo <> MatrixRec."Job No." then MatrixCellQuantity+=1; //Round(MatrixRec.Quantity * MatrixRec."Qty. per Unit of Measure", UOMMgt.QtyRndPrecision())
                    JobNo:=MatrixRec."Job No.";
                until MatrixRec.Next() = 0;
            if MatrixCellQuantity <> 0 then MatrixCellData[I]:=Format(MatrixCellQuantity)
            else
                MatrixCellData[I]:='';
        end;
        SetVisible();
    ...
    begin
        ...
        if JobPlanningLine.FindSet()then repeat if Job.get(JobPlanningLine."Job No.")then job.Mark(true);
    ...
    local procedure GetStartDate(PeriodStartDate: Date): Date begin
        case AmountType of AmountType::"Net Change": exit(PeriodStartDate);
        AmountType::"Balance at Date": exit(0D);
        end;
    end;
